...
type PacketAttempt struct {
	...
	// response. They are sanitized before being written.
	Reasoning        string
	...
	// Provider interaction artifacts. Prompt and Schema are echoed as-is;
	// Evidence is sanitized; Response.JSON is sanitized and parsed back to
	// JSON for embedding (or wrapped as {raw_text:...} if redaction breaks
	// JSON validity).
	Prompt          string
	Schema          *Schema
	Evidence        string
	...
	// (e.g. GitHub tokens read from config.Vars) so the sanitizer redacts
	// them anywhere they appear in the packet.
...
// honoring the SDK config's Write flag. It returns nil (no-op) when writes
// are disabled, when WriteDirectory is empty, or when ControlID is empty.
//
...
//
// All redaction is performed before bytes are written to disk. Configured
// secret values (APIKey, BaseURL credentials, sensitive config.Vars, and
// any ExtraSecretValues supplied by the caller) are removed by exact-value
// replacement; a pattern-based scrubber catches bearer tokens and other
// token-shaped values that slipped through in evidence or model output.
func WritePacket(config sdkconfig.Config, attempt PacketAttempt) error {
	...
	packetDir := filepath.Join(
		...
		"ai-evidence",
		controlID,
		packetDirectoryName(packetRequestID(attempt.Response)),
	)
	...
	assessmentFile := AssessmentFile{
		...
		Provider:          packetProvider(attempt.Response),
		Model:             packetModel(attempt.Response),
	...
	if err := writePacketJSON(filepath.Join(packetDir, "assessment.json"), assessmentFile); err != nil {
		return err
	}

	interaction := buildAIInteraction(attempt, sanitizer)
	if err := writePacketJSON(filepath.Join(packetDir, "ai_interaction.json"), interaction); err != nil {
		return err
	}
	return nil
}
...
func buildAIInteraction(attempt PacketAttempt, sanitizer Sanitizer) AIInteractionFile {
	file := AIInteractionFile{
		...
		Evidence: AIInteractionEvidence{
			Sources: append([]string(nil), attempt.EvidenceSources...),
...
func packetProvider(response *AnalyzeResponse) Provider {
	if response == nil {
		return ""
	}
...
func packetModel(response *AnalyzeResponse) string {
	if response == nil {
		return ""
	}
...
// It performs exact-value replacement for known secrets gathered from
// config plus pattern-based redaction for token-shaped strings that may
// have leaked through evidence or model output.
type Sanitizer struct {
	secretValues []string
}

// NewSanitizer assembles a Sanitizer for the given SDK config. It extracts
// known secret values (AI API key, credentials embedded in ai_base_url,
...
// directly to redact log output before WritePacket runs.
...
// RedactBytes is the []byte form of RedactText, returning nil on empty input.
func (s Sanitizer) RedactBytes(value []byte) []byte {
	if len(value) == 0 {
		return nil
	}
	return []byte(s.RedactText(string(value)))
}
...
func writePacketJSON(path string, value interface{}) error {
	var body []byte
	var err error
