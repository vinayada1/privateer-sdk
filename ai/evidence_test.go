...
func TestWritePacketSucceededAttempt(t *testing.T) {
	...
	attempt := PacketAttempt{
		...
		Prompt:          "You are assessing test execution documentation.",
		Schema: &Schema{
...
func TestRedactPatterns(t *testing.T) {
	...
	for _, leaked := range []string{"sk-live-1234567890abcdef", "plain-secret", "ghp_secret_12345678"} {
		if strings.Contains(got, leaked) {
			t.Fatalf("RedactPatterns leaked %q in %q", leaked, got)
		}
	}
	for _, want := range []string{"Authorization: Bearer REDACTED", "api_key=REDACTED", "token: REDACTED"} {
